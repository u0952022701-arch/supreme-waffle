...
if st.button("開始計算支撐壓力", type="primary"):
    ...
    if df.empty:
        ...
        high_vol_closes = df[df['Volume'] > df['Vol_MA20'] * 1.8]['Close'].tolist()  # 抓取高於20日均量1.8倍的K線收盤價

        # 波段極值
        order = 5 # Window size for local extrema
        ...
        support_pool = [x for x in support_pool if not pd.isna(x) and x < current_price] # 過濾無效值和高於現價的支撐

        resistance_pool = high_vol_closes + local_maxs
        resistance_pool = [x for x in resistance_pool if not pd.isna(x) and x > current_price] # 過濾無效值和低於現價的壓力
        ...
        second_res = None
        if first_res: # 如果有第一壓力，才找第二壓力
            second_res_candidates = [x for x in res_clusters if x > first_res * 1.03] # 尋找比第一壓力高3%以上的壓力
